...
public class DownloadManager implements DTListener, PeerUpdateListener, ConListenerInterface
{
	...
	private Vector<String> peerListStr = new Vector<String>();
	...
	private float							lastMb				= 0;
	private int								lastCount			= 0;

	private float							lastMbSec			= 0;
	 ...
	 *            byte[]
	 */
	public DownloadManager(TorrentFile torrent, final byte[] clientID, JList list, JLabel label, JProgressBar prog)
	{
		this.list = list;
		this.rate = label;
		...
		this.rate.setText("Проверка загруженных файлов...");
	...
	public synchronized int checkTempFiles()
	{
		...
		for (int i = 0; i < this.nbOfFiles; i++)
		{
			...
			if (this.nbOfFiles >1)
			{
				String name = ((String) (this.torrent.name.get(i)));
				System.out.println("FILE NAME SAVE: "+saveas + name.substring(name.lastIndexOf("/")+1));
				temp = new File(saveas + name.substring(name.lastIndexOf("/")+1));
			}
			else
			{
				temp = new File(saveas + ((String) (this.torrent.name.get(i))));
			}
	...
	public synchronized void taskCompleted(String id, int reason)
	{
		switch (reason)
		{
			...
			case DownloadTask.UNKNOWN_HOST:
				// System.err.println("Connection could not be established to "
				// + id + ". Host unknown...");
	...
	public synchronized void pieceCompleted(String peerID, int i, boolean complete)
	{
		...
		synchronized (this.isComplete)
		{
			if (complete && !this.isPieceComplete(i))
			{
				...
				// + " (Total dl = " + totaldl + "% )");
				// this.torrent.total_length
				// this.torrent.
				// this.rate.setText(Float.toString(this.getDLRate()));
				// System.out.println(getDLRate());
				this.progressBar.setValue((int) totaldl);
				...
				if (this.lastCount > 10)
				{
					...
					float currentMb = (((float) (this.isComplete.cardinality())) * (this.torrent.total_length / ((float) (this.nbPieces))) / (1024)) / 1024;

					aa = ((currentMb - this.lastMb) / (currentTime - this.lastTime)) * 1000;

					this.lastTime = currentTime;
					this.lastMb = currentMb;
					this.lastCount = 0;
				}
				else
				{
				...
				String cur = Float.toString((((float) (this.isComplete.cardinality())) * (this.torrent.total_length / ((float) (this.nbPieces))) / (1024)) / 1024);

				if (cur.indexOf(".") + 2 < cur.length())
				{
					cur = cur.substring(0, cur.indexOf(".") + 2);
				}

				String len = Float.toString((this.torrent.total_length / (1024)) / 1024);
				if (len.indexOf(".") + 2 < len.length())
				{
					len = len.substring(0, len.indexOf(".") + 2);
				}

				String dl = Float.toString(totaldl);
				if (dl.indexOf(".") + 3 < dl.length())
				{
					dl = dl.substring(0, dl.indexOf(".") + 3);
				}
				...
				String mbsec = " ~ " + Float.toString(this.lastMbSec);
				if (mbsec.indexOf(".") + 4 < mbsec.length())
				{
					mbsec = mbsec.substring(0, mbsec.indexOf(".") + 4);
				}
				mbsec += "МиБ/сек";

				this.rate.setText(cur + "МиБ из " + len + "МиБ (" + dl + "% )" + mbsec);
			...
			if (this.isComplete.cardinality() == this.nbPieces)
			{
				...
				// this.task.clear();
				// this.end();
				rate.setText("Загрузка завершена");
	...
	private synchronized void unchokePeers()
	{
		...
		synchronized (this.task)
		{
			...
			for (Iterator it = l.iterator(); it.hasNext();)
			{
				...
				if (p.getDLRate(false) > 0)
				{
					...
					// (1024 * 10) + "ko/s");
					sumDL += p.getDLRate(false) / (1024 * 10);
					sum++;
				}
				...
				if (nbDownloaders < 5 && dt != null)
				{
					...
					if (!p.isInterested())
					{
						...
						// if (p.isChoked())
						// dt.ms.addMessageToQueue(new
						// Message_PP(PeerProtocol.UNCHOKE));
						p.setChoked(false);
			...
			// rate.setText("Пир:" + Integer.toString(sum) + "   " +
			// Float.toString(getDLRate()*8));
		}
		this.lastUnchoking = System.currentTimeMillis();
		...
		if ((int) totaldl == 100)
		{
			this.rate.setText("100% - Torrent раздаётся");
			this.rate.repaint();
			this.rate.validate();
		}
	}
	...
	public synchronized void peerRequest(String peerID, int piece, int begin, int length)
	{
		if (this.isPieceComplete(piece))
		{
			...
			if (dt != null)
			{
				dt.ms.addMessageToQueue(new Message_PP(PeerProtocol.PIECE, Utils.concat(Utils.intToByteArray(piece), Utils.concat(Utils.intToByteArray(begin), this.getPieceBlock(piece, begin, length)))));
		...
		synchronized (this.task)
		{
			...
			for (Iterator i = keyset.iterator(); i.hasNext();)
			{
				...
				if (!this.task.containsKey(key))
				{
					...
					peerListStr.add(p.toString());
					this.peerList.put(p.toString(), p);
					this.connect(p);
				}
			}
		}
		countPeers = peerList.size();
	...
	public float getCompleted()
	{
		try
		{
			return (float) (((float) (100.0)) * ((float) (this.isComplete.cardinality())) / ((float) (this.nbPieces)));
		}
		catch (Exception e)
		{
			return 0.00f;
